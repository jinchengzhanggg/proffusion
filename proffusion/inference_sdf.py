...
parser = ArgumentParser(description="inference a Proffusion model")
...
def get_data_preprocessed(song, data_type, song_fn):
    prmat2c, pnotree, chord, prmat = song.get_whole_song_data()

    if prmat2c.numel() == 0:
        print(f"Skipping {song.song_fn} due to empty prmat2c data.")
    ...
    if chord is not None:
        ...
        # *************************************************************************************
        for i, (prmat2c_seg, pnotree_seg, chord_seg, prmat_seg) in enumerate(zip(prmat2c_np, pnotree_np, chord_np, prmat)):
            prmat2c_seg = np.expand_dims(prmat2c_seg, axis=0)
            pnotree_seg = np.expand_dims(pnotree_seg, axis=0)
            chord_seg = np.expand_dims(chord_seg, axis=0)
            prmat_seg = np.expand_dims(prmat_seg, axis=0)
            prmat2c_to_midi_file(prmat2c_seg, f"exp/{data_type}_{song_fn}_prmat2c_segment_{i}.mid")
            estx_to_midi_file(pnotree_seg, f"exp/{data_type}_{song_fn}_pnotree_segment_{i}.mid")
            chd_to_midi_file(chord_seg, f"exp/{data_type}_{song_fn}_chord_segment_{i}.mid")
            prmat_to_midi_file(prmat_seg, f"exp/{data_type}_{song_fn}_prmat_segment_{i}.mid")
        # *************************************************************************************
        return (
            prmat2c.to(device),
            pnotree.to(device),
        ...
        # *************************************************************************************
        for i, (prmat2c_seg, pnotree_seg, prmat_seg) in enumerate(zip(prmat2c_np, pnotree_np, prmat)):
            prmat2c_seg = np.expand_dims(prmat2c_seg, axis=0)
            pnotree_seg = np.expand_dims(pnotree_seg, axis=0)
...
def choose_song_from_val_dl(data_type, i, use_track=[0, 1, 2]):
    split_fpath = join(TRAIN_SPLIT_DIR, "pop909.pickle")
    with open(split_fpath, "rb") as f:
        split = pickle.load(f)
    print(split[1])
    num_songs = len(split[1])
    num = i % num_songs
    ...
    return *get_data_preprocessed(song, data_type, song_fn), song_fn
...
class Experiments:
    ...
    def generate(
        ...
        if not no_output:
            ...
            for i, segment in enumerate(gen):
                ...
                segment_np = np.expand_dims(segment_np, axis=0)
                segment_output_path = os.path.join(output_dir, f"{output_stamp}_segment_{i}.mid")
                prmat2c_to_midi_file(segment_np, segment_output_path)
    ...
    def inpaint(
        ...
        if not no_output:
            ...
            mask = mask.cpu().numpy()
            mel = prmat2c*mask
            for i, (prmat2c_seg,mel_seg) in enumerate(zip(prmat2c,mel)):
                prmat2c_seg = np.expand_dims(prmat2c_seg, axis=0)
                mel_seg = np.expand_dims(mel_seg, axis=0)
                prmat2c_to_midi_file(prmat2c_seg, os.path.join(output_dir, f"{output_stamp}_segment_{i}.mid"))
                prmat2c_to_midi_file(mel_seg, os.path.join(output_dir, f"mel_{output_stamp}_segment_{i}.mid"))
            ...
            # mel = prmat2c*mask
            # prmat2c_to_midi_file(
            #     mel, os.path.join(output_dir, f"melody_{output_stamp}.mid")
            # )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_generate", default=89, help="the number of samples to generate"
    )
    parser.add_argument(
    ...
    if args.custom_params_path is None:
        ...
        print(f"{model_path}/params.yaml")
    ...
    for i in range(int(args.num_generate)):
        ...
        if float(args.uncond_scale) == 0.0:
            ...
            if args.from_midi is not None:
                ...
                prmat2c, pnotree, chd, prmat, song_fn = choose_song_from_val_dl("cond", i)
                if prmat2c is None:
                    print(f"Skipping {song_fn} due to missing or empty data.")
                    continue
            else:
                raise NotImplementedError
