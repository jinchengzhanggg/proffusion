...
class LWN(nn.Module):
    ...
    def get_wavelet_loss(self):
        # if torch.isnan(self.perfect_reconstruction_loss()[0]).any():
        #     self.perfect_reconstruction_loss()[0] = 0.0000001
        # if torch.isnan(self.alias_cancellation_loss()[0]).any():
        #     self.alias_cancellation_loss()[0] = 0.0000001
        # return self.perfect_reconstruction_loss()[0] + self.alias_cancellation_loss()[0]

        # perfect_loss = self.perfect_reconstruction_loss()[0]
        # alias_loss = self.alias_cancellation_loss()[0]

        # if torch.isnan(perfect_loss).any():
        #     perfect_loss = torch.tensor(1e-7, device=perfect_loss.device, dtype=perfect_loss.dtype)
        # if torch.isnan(alias_loss).any():
        #     alias_loss = torch.tensor(1e-7, device=alias_loss.device, dtype=alias_loss.dtype)

        # return perfect_loss + alias_loss
        return self.perfect_reconstruction_loss()[0] + self.alias_cancellation_loss()[0]
...
if __name__ == '__main__':
    ...
    Model = WaveletBlock(c = 256)
    # Model=LWN(use_ca=True, dim=512)
    output = Model(input)
    print(output.shape)
